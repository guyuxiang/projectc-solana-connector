...
BUILD_FOLDER = ./build
PROJECT_URL  = github.com/guyuxiang/projectc-solana-connector
## command
GO           = go
GO_VENDOR    = go mod
MKDIR_P      = mkdir -p
...
dockerfiles.build:
	docker build --no-cache --rm --tag guyuxiang/projectc-solana-connector:$(SERVER_VERSION) -f ./docker/Dockerfile .
